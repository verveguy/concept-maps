...
export function useCommentActions() {
   ...
   * automatically generated unless provided. Timestamps (`createdAt`, `updatedAt`) are set to
   * the current time.
   * 
   ...
   * @param commentId - Optional comment ID to use (if not provided, one will be generated)
   * 
   ...
   * ```
   */
  const createComment = async (comment: CreateCommentData, commentId?: string) => {
    if (!auth.user?.id) {
